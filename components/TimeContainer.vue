<template>
    ...
    >
      ...
      >
        ...
        >
          ...
          <div class="w-full bg-[#F3F4F6] rounded-lg h-12 dark:bg-gray-700">
            <div
  ...
  const updateProgressBar = (): void => {
    ...
    const startTimeInMinutes = confirmedValue.value * 60 + confirmedValue2.value;
  ...
  const checkLocalStorage = () => {
    ...
    if (storedStartHH && storedStartMM && storedDate === currentDate) {
      ...
      timeValue.value = `${confirmedValue.value
        .toString()
        .padStart(2, '0')}:${confirmedValue2.value.toString().padStart(2, '0')}`;
      updateProgressBar();
      computeEndTime(confirmedValue.value, confirmedValue2.value);
      computeTimeDelta();
    } else {
